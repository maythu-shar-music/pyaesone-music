...
from pyaesonemusic.misc import SUDOERS
from pyaesonemusic.core.mongo import mongodb
from pyaesonemusic.utils.errors import capture_err
from pyaesonemusic.utils.keyboard import ikb
from pyaesonemusic.utils.database import save_filter
from pyaesonemusic.utils.functions import (
    extract_user,
    extract_user_and_reason,
    time_converter,
)
from pyaesonemusic.utils.permissions import adminsOnly, member_permissions
...
@Client.on_message(filters.command(["kick", "skick"]) & ~filters.private & ~BANNED_USERS)
...
@Client.on_message(filters.command("unban") & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_restrict_members")
...
@Client.on_callback_query(filters.regex(r"^check_powers_(\d+)"))
...
# Toggle admin power
@Client.on_callback_query(filters.regex(r"^toggle_(.+)_(\d+)"))
...
@Client.on_message(filters.command("demote") & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_promote_members")
...
@Client.on_message(filters.command(["unpinall"]) & filters.group & ~BANNED_USERS)
...
@Client.on_callback_query(filters.regex(r"unpin_(yes|no)"))
...
@Client.on_message(filters.command(["pin", "unpin"]) & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_pin_messages")
async def pin(_, message: Message):
    if not message.reply_to_message:
...
@Client.on_message(filters.command(["mute", "tmute"]) & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_restrict_members")
...
@Client.on_message(filters.command("unmute") & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_restrict_members")
...
@Client.on_message(filters.command(["warn", "swarn"]) & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_restrict_members")
...
@Client.on_callback_query(filters.regex("unwarn_") & ~BANNED_USERS)
...
@Client.on_message(filters.command("rmwarns") & ~filters.private & ~BANNED_USERS)
@adminsOnly("can_restrict_members")
...
@Client.on_message(filters.command("warns") & ~filters.private & ~BANNED_USERS)
@capture_err
...
@Client.on_message(filters.command("unbanme"))
