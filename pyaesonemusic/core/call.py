...
from pyaesonemusic import LOGGER, YouTube, app
from pyaesonemusic.misc import db
from pyaesonemusic.utils.database import (
    add_active_chat,
...
from pyaesonemusic.utils.exceptions import AssistantErr
from pyaesonemusic.utils.formatters import check_duration, seconds_to_min, speed_converter
from pyaesonemusic.utils.inline.play import stream_markup
from pyaesonemusic.utils.stream.autoclear import auto_clean
from pyaesonemusic.utils.thumbnails import get_thumb
from pyaesonemusic.utils.errors import capture_internal_err, send_large_error
...
class Call:
    def __init__(self):
        self.userbot1 = Client(
            "pyaesonemusic1", config.API_ID, config.API_HASH, session_string=config.STRING1
        ) if config.STRING1 else None
        self.one = PyTgCalls(self.userbot1) if self.userbot1 else None

        self.userbot2 = Client(
            "pyaesonemusic2", config.API_ID, config.API_HASH, session_string=config.STRING2
        ) if config.STRING2 else None
        self.two = PyTgCalls(self.userbot2) if self.userbot2 else None

        self.userbot3 = Client(
            "pyaesonemusic3", config.API_ID, config.API_HASH, session_string=config.STRING3
        ) if config.STRING3 else None
        self.three = PyTgCalls(self.userbot3) if self.userbot3 else None

        self.userbot4 = Client(
            "pyaesonemusic4", config.API_ID, config.API_HASH, session_string=config.STRING4
        ) if config.STRING4 else None
        self.four = PyTgCalls(self.userbot4) if self.userbot4 else None

        self.userbot5 = Client(
            "pyaesonemusic5", config.API_ID, config.API_HASH, session_string=config.STRING5
